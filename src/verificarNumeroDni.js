...
//const numeroDni="0590p600L";

function validarNumeroDni(numeroDni) {

   const onlyLetter= numeroDni[numeroDni.length-1].toUpperCase();
   const onlyNumbers = parseInt(numeroDni.slice(0, numeroDni.length - 1));
   let mensaje=calcularLetraDNI(onlyNumbers,onlyLetter);
   if (onlyNumbers < 0 || onlyNumbers > 99999999 || numeroDni.length > 9) mensaje = "El numero de su DNI no es correcto";
   return mensaje; 
...
function calcularLetraDNI(numeroCorrecto,letra)
{
  ...
  mensaje="La letra que introdujo no es la correcta";
  if (letra == letrasDNI[resto]) mensaje= "Esta correcto el DNI";
  return mensaje;
...
let resul=validarNumeroDni(numeroDni);
console.log(resul);
